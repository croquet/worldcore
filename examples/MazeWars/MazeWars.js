...
import { App, Session, Constants, View } from "@croquet/croquet";
import { mix, ViewRoot, ModelRoot, WebInputManager, Actor, Pawn, AM_MouselookAvatar, PM_MouselookAvatar, GetNamedView, AM_Avatar, PM_Avatar } from "@croquet/worldcore";
...
import eyeball_fbx from "./assets/eyeball.fbx";
let eyeball_D = "./assets/textures/Eye_D.jpg";
let eyeball_N = "./assets/textures/Eye_N.jpg";
let explosion = "./assets/textures/explosion.png";
...
class Avatar3DGenerator {
 ...
 async eyeConstruct() {
    ...
    let eye_D = new THREE.TextureLoader().load( eyeball_D );
    let eye_N = new THREE.TextureLoader().load( eyeball_N );
    let fbxLoader = new FBXLoader();
    // load the 3D model
    let eyeball = await new Promise( (resolve, reject) => fbxLoader.load(eyeball_fbx, resolve, null, reject) );
      ...
      eyeball.traverse( child => {
        if ( child.isMesh ) {
          ...
          mat.emissive = child.material.emissive;
          mat.shininess = child.material.shininess;
          mat.color = child.material.color;
          if (child.material.map)mat.map = eye_D;
          if (child.material.normalMap)mat.normalMap = eye_N;
          mat.emissiveIntensity = 0.55;
          child.material = mat;
        }
      //} );
      eyeball.scale.set(0.0012, 0.0012, 0.0012);
      this.avatar3D = eyeball;
    });
  }
...
async function go() {
  ...
  await Session.join(`mazewars-${App.autoSession()}`, MazeModel, MazeView, { tps: 20 });
